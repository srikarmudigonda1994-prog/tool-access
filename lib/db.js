...
async function initSchema() {
  ...
  // so this works whether the table is brand new or already existed before
 ...
 */
async function createCustomer(name, tools, email, phone) {
  const db = getPool();
  const token = generateToken();
  const toolsStr = tools.join(',');
  const result = await db.query(
    `INSERT INTO customers (token, name, tools, email, phone) VALUES ($1, $2, $3, $4, $5)
     RETURNING id, token, name, tools, email, phone, created_at, revoked`,
    [token, name, toolsStr, email || null, phone || null]
  );
...
async function findCustomerByEmail(email) {
  const db = getPool();
  const result = await db.query(
    `SELECT id, token, name, tools, email, phone, created_at, revoked
...
/**
 * Grants (or updates) a person's tool access, keyed by email. If a
 * customer record already exists for that email, their tools and name
 * are updated and their existing link/token stays the same. Otherwise a
 * brand new customer + token is created. Either way you get back one
 * consistent link to share.
 */
async function upsertCustomerAccess({ name, email, phone, tools }) {
  const existing = email ? await findCustomerByEmail(email) : null;
  if (existing) {
    const db = getPool();
    const toolsStr = tools.join(',');
    const result = await db.query(
      `UPDATE customers SET name = $1, tools = $2, phone = $3, revoked = false
       WHERE token = $4
       RETURNING id, token, name, tools, email, phone, created_at, revoked`,
      [name, toolsStr, phone || null, existing.token]
    );
    return { customer: result.rows[0], isNew: false };
  }
  const customer = await createCustomer(name, tools, email, phone);
...
async function updateCustomerTools(token, tools) {
  const db = getPool();
  const toolsStr = tools.join(',');
  const result = await db.query(
    `UPDATE customers SET tools = $1 WHERE token = $2
     RETURNING id, token, name, tools, created_at, revoked`,
    [toolsStr, token]
  );
  return result.rows[0] || null;
